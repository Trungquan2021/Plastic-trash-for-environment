...
const SESSION_COOKIE_NAME = 'pte_sid';
...
function normalizeDb(raw = {}) {
  return {
    ...
    photoHashes: Array.isArray(raw.photoHashes) ? raw.photoHashes : [],
    cameraChallenges: Array.isArray(raw.cameraChallenges) ? raw.cameraChallenges : [],
    counters: {
      users: Number(raw.counters?.users || 0),
      submissions: Number(raw.counters?.submissions || 0)
    }
  };
}
...
function readDb() {
  if (!fs.existsSync(DB_FILE)) {
    const seed = normalizeDb({});
    fs.writeFileSync(DB_FILE, JSON.stringify(seed, null, 2), 'utf8');
    return seed;
  }
  return normalizeDb(safeJsonRead(DB_FILE, {}));
}
...
function sanitizeUser(user) {
  ...
  return {
    ...
    recentSubmissions: Array.isArray(user.recentSubmissionIds) ? user.recentSubmissionIds.length : 0
  };
}
...
function decodeSessionCookie(rawCookie) {
  ...
  if (!timingSafeEqualString(signSessionId(sid), sig)) return null;
  return sid;
}
...
function clearSessionById(db, sessionId) {
...
function getCurrentSession(req, db) {
  ...
  const sessionId = decodeSessionCookie(signedCookie);
...
function currentUser(req) {
  ...
  return user ? { user, session, db } : null;
}
...
function uniqueUsername(db, preferred) {
  let base = slugify(preferred) || 'nguoi-dung';
  let candidate = base;
  let i = 2;
  while (db.users.some((u) => String(u.username || '').toLowerCase() === candidate.toLowerCase())) {
    candidate = `${base}-${i++}`;
  }
  return candidate;
}
...
app.get('/api/health', (_req, res) => {
  const writable = fs.existsSync(DATA_DIR);
  res.json({
    ok: true,
    appOrigin: APP_ORIGIN,
    auth: { local: true },
    storage: { type: 'json-file', writable },
    time: nowIso()
  });
});
...
app.get('/api/auth/me', (req, res) => {
  const current = currentUser(req);
  const safeUser = sanitizeUser(current?.user || null);
  const level = safeUser ? calculateLevel(safeUser.stats.puzzles) : null;
  res.json({ ok: true, user: safeUser, level });
});
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    if (!safeName || !safeEmail || safePassword.length < 8) {
      return res.status(400).json({ ok: false, message: 'Vui lòng nhập đủ họ tên, email và mật khẩu từ 8 ký tự.' });
    }
    if (!emailOk) {
      return res.status(400).json({ ok: false, message: 'Email chưa đúng định dạng.' });
    }
    ...
    const safeUsername = uniqueUsername(db, String(username || safeEmail.split('@')[0] || safeName));
    ...
    res.status(500).json({ ok: false, message: error.message || 'Không thể tạo tài khoản lúc này.' });
  }
});
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    const safeIdentifier = String(identifier || email || '').trim().toLowerCase();
    const safePassword = String(password || '');
    const db = readDb();
    const user = db.users.find((u) => {
      const userEmail = String(u.email || '').toLowerCase();
      const userName = String(u.username || '').toLowerCase();
      return userEmail === safeIdentifier || userName === safeIdentifier;
    });

    if (!user || !user.passwordHash) {
      return res.status(401).json({ ok: false, message: 'Sai email/tên đăng nhập hoặc mật khẩu.' });
    }

    const match = await verifyPassword(safePassword, user.passwordHash);
    if (!match) {
      return res.status(401).json({ ok: false, message: 'Sai email/tên đăng nhập hoặc mật khẩu.' });
    }
    ...
    res.status(500).json({ ok: false, message: error.message || 'Không thể đăng nhập lúc này.' });
  }
});
...
app.post('/api/auth/logout', (req, res) => {
  ...
  const session = getCurrentSession(req, db);
  if (session) {
    clearSessionById(db, session.id);
    writeDb(db);
  }
...
app.post('/api/submissions', requireAuth, async (req, res) => {
  try {
    ...
    const db = req.db;
    trimExpiredChallenges(db);
    const challenge = db.cameraChallenges.find((c) => c.id === challengeId && c.userId === req.user.id && c.sessionId === req.sessionInfo.id);
