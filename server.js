...
function getGoogleCallbackUrl(req) {
  return (process.env.GOOGLE_CALLBACK_URL || `${getRequestOrigin(req)}/auth/google/callback`).replace(/\/$/, '');
}

function getFacebookCallbackUrl(req) {
  return (process.env.FACEBOOK_CALLBACK_URL || `${getRequestOrigin(req)}/auth/facebook/callback`).replace(/\/$/, '');
}
...
function upsertOAuthUser({ provider, providerId, name, email }) {
  ...
  if (!user && email) user = db.users.find((u) => u.email && u.email.toLowerCase() === email.toLowerCase());
  if (!user) {
    user = {
...
if (GOOGLE_ENABLED && process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_SECRET) {
...
if (FACEBOOK_ENABLED && process.env.FACEBOOK_APP_ID && process.env.FACEBOOK_APP_SECRET) {
...
function inferObjects(clientMetrics = {}, metrics = {}) {
  const objects = [];
  const lowerHints = Array.isArray(clientMetrics.objectHints) ? clientMetrics.objectHints.map((v) => String(v || '').toLowerCase()) : [];
...
app.get('/api/health', (req, res) => {
  res.json({
    ...
    auth: {
      google: GOOGLE_ENABLED && Boolean(process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_SECRET),
      facebook: FACEBOOK_ENABLED && Boolean(process.env.FACEBOOK_APP_ID && process.env.FACEBOOK_APP_SECRET),
      local: LOCAL_ENABLED
    }
  });
});
...
app.get('/api/config', (req, res) => {
  res.json({
    ...
    facebookCallbackUrl: getFacebookCallbackUrl(req),
...
app.get('/auth/google/callback', (req, res, next) => {
  ...
  passport.authenticate('google', { callbackURL: getGoogleCallbackUrl(req) }, (err, user) => {
...
app.get('/auth/facebook/callback', (req, res, next) => {
  ...
  passport.authenticate('facebook', { callbackURL: getFacebookCallbackUrl(req) }, (err, user) => {
...
app.listen(PORT, () => {
  console.log(`Plastic Trash for Environment listening on port ${PORT}`);
});
